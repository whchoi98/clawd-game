 ...
 * real one (public/ + src/client/main.ts + src/server/index.ts). The script is
 * run as a child process, exactly the way `npm run build` runs it.
 */
...
function scaffold(dir: string, names = { pub: 'public', client: 'src/client/main.ts', server: 'src/server/index.ts' }): void {
  mkdirSync(join(dir, names.pub), { recursive: true });
...
describe('tools/build.mjs', () => {
  ...
  it('is deterministic: rebuilding identical sources yields identical asset names', () => {
    const root = join(dir, 'defaults');
  ...
  it('honours explicit env paths and --prod (minified, NODE_ENV=production)', () => {
    ...
    scaffold(root, { pub: 'web', client: 'app/entry.ts', server: 'srv/boot.ts' });
  ...
  it('replaces stale hashed assets instead of accumulating them', () => {
    const root = join(dir, 'defaults');
