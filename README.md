...
플레이어는 뾰족한 귀와 말린 꼬리를 가진 **고양이 클로드**입니다. 기본 모습에는 민트색 스카프를 둘렀고, 여덟 가지 스킨·메아리·공유 카드에도 같은 고양이가 등장합니다.
