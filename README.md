...
첫 화면의 **설정 → 캐릭터**에서 **고양이·토끼·로봇**을 처음부터 고를 수 있습니다. 고양이 클로드는 뾰족한 귀와 말린 꼬리, 토끼는 긴 귀와 둥근 꼬리, 로봇은 사각 몸체와 LED 얼굴을 가집니다. 세 캐릭터는 같은 이동·점프·대시 규칙으로 탑을 오릅니다.

기본 캐릭터 3종과 해금하는 고양이 모습 7종을 합쳐 **총 10가지 모습**을 제공합니다. 선택은 저장되며 타이틀·플레이·메아리·대시 잔상·공유 카드에 반영됩니다. 기존에 얻은 모습과 기록도 계속 사용할 수 있습니다.
...
모드는 세 가지입니다. **탑 오르기**(4개 층 × 4구역 = 16구역 — 층마다 가로 방 3개와 **세로 존** 1개, 4층 '오로라 정점'은 Phase 5), **데일리 타워**(서버가 매일 UTC 자정에 발급하는 시드로 모두가 같은 탑을 오르고 세계 순위를 겨룸), **끝없는 등반**(차오르는 조류, 개인 기록).
...
- **오디오 연출 패스·캐릭터 주스·세레머니** (P3-9, `src/client/audio/`, `src/client/render/clawd.ts`·`particles.ts`, `src/client/ui/ceremony.ts`) — 골 터치는 재생 중인 바이옴 트랙의 조성으로 쓴 **클리어 스팅어**(E 도리안 마림바 / E 프리지안 스탭 / A 에올리안 벨)를 내고, 체크포인트 차임은 순번마다 한 음씩 오르며, 파편 콤보는 5음계 사다리를 타다 8콤보부터 5도가 겹칩니다. 층 돌파 팡파르·엔딩 화음·별/메달 사운드는 `AudioEngine.stinger`로 sim 이벤트 밖에서 울리고, 엔딩 화면은 타이틀 테마의 변주 트랙 `ending`(F 리디안·64 bpm·글라스 패드)으로 크로스페이드됩니다. 캐릭터는 착지 임팩트에 비례한 먼지, 스킨 색의 대시 잔상 실루엣, 벽 슬라이드 스파크, 골 컨페티, 리스폰 팝을 얻고 골이 6타일 안이면 눈동자가 골을 봅니다(`setLookTarget`, 라이브 플레이어만). 결과 화면은 `Timeline`이 등급 → 별 → 메달 행(`LevelRecord.medals`) → 보드 순으로 `data-stage`를 올려 CSS가 단계별로 드러내며(아무 키로 완료, `prefers-reduced-motion`과 `?shot=` 하네스는 즉시), 한 층의 마지막 구역을 처음 돌파하면 결과 전에 3초짜리 **층 돌파 비스타 카드**(`#scr-tier`, 바이옴 그라디언트·로마 숫자·한 줄 서사·다음 층, 아무 키 스킵)가 한 번 나오고 `Progress.tiersBroken`에 남습니다. 16개 구역을 모두 처음 돌파하면 결과 대신 **엔딩**(`#scr-ending`: 절차적 밤하늘 캔버스, 정상의 클로드, 3줄 서사, 총 별·유물·사망·최고 기록 합, 이 클리어의 제출 줄, '다시 오르기' → 탑)이 한 번 나오고 `Progress.endingSeen` 이후 타이틀에 한 줄 서사가 붙습니다. 두 플래그는 이전 코드 스냅샷에 실려 병합되고, 진행 기록 삭제로 지워집니다. 텔레메트리는 기존 화면 이름만 씁니다(엔딩은 `result`).
...
- **배경 강화** (P5-3, `src/client/render/sky.ts`·`tiles.ts`, 렌더 전용 — sim과 리플레이는 모릅니다) — 스카이는 하늘 그라디언트 → 별 → **오로라 리본**(정점, fbm 흐름) → 천체 → 구름 띠 → 원경 능선 → **원경 구조물**(등대·난파선·야자 / 창불 켜진 첨탑 / 해파리·산호 아치 / 눈 봉우리·부유 사당) → 중경 능선·소품 → 근경 능선 → **구름 갑판**(세로 구역) → **동물 무리**(갈매기·박쥐·빛벌레·눈바다제비, 화면 상단 밴드 한정) → 배경 절벽 → 안개 → 날씨(물보라·비·포자·**눈**) 순서로 전부 월드 변환 앞에서 그려져 어떤 층도 플레이필드 위에 올라오지 않습니다. 세로 구역(`pxH > pxW`)은 높이 55 % 지점에 구름층이 있어 뚫고 오르면 별이 짙어지고 발밑이 구름 바다가 되며(`Sky.deckFactor`, camY에 단조), 가로 구역은 진행도(camX/폭)에 따라 하늘 그라디언트가 최대 0.15만큼 다음 정지점 색으로 번져 시간이 흐르는 느낌을 냅니다. 지형은 노출된 면에만 바이옴 장식(따개비·이끼 / 발광 룬 / 결정 / 얼음 광택·눈 모자)을 얹는데, `tileDecor(biome, tx, ty, faces)`가 타일 좌표만으로 결정하고 종류별 Path2D 하나로 모아 한 번씩 채우므로 프레임당 지형 fill은 최대 9회입니다. 품질 티어 `low`는 새 층을 모두 끄고 `balanced`는 무리를 절반으로 줄이며 `Sky.counters`가 층별 호출 수를 셉니다. `npm run qa:readability`는 기존 3단계에 `backdrop:t1/s1/v1`(스카이만 다시 그려 플레이필드 밴드의 픽셀 열 평균이 crust 루마를 넘지 않음)과 `tier4-*`(정점 구역용, P5-1 병합 후 상수 전환) 단계를 더해 9단계를 검사합니다. 데일리·끝없는 등반(조류 레벨)에서는 렌더러가 매 프레임 플레이어 발 밑 행의 밴드(`bandBiome`, HUD 라벨과 같은 호출)를 따라가 밴드가 바뀌면 1.2 s(`BAND_FADE_S`) 크로스페이드로 스카이·지형 팔레트·안개·비콘을 새 밴드로 넘깁니다(P5-4b): 같은 seed·레벨 박스로 만든 스카이 2개를 `Sky.opacity`로 겹치므로 능선·절벽·별 실루엣은 그대로고 색·날씨·소품만 바뀌며, 밴드별 `Terrain`은 setLevel에서 미리 베이킹됩니다. 경계는 진행 방향으로 1타일 히스테리시스, 리스폰·텔레포트(`?shot=daily&at=`)와 `low` 티어는 하드컷, 스토리 구역은 `def.biome` 고정입니다.
## Phase 5 A: 캐릭터 강화·정점 음악

- **리그 표정·2차 모션·액세서리·고양이 스킨 8종·적 예고 포즈·정점 트랙** (P5-2, `src/client/render/clawd.ts`·`actors.ts`, `src/client/unlocks.ts`, `src/client/audio/music.ts`·`sfx.ts`) — 클로드의 얼굴이 포즈를 따라갑니다(`expressionFor`): 달리면 눈썹이 모이고, 떨어지면 눈썹이 오르며 입이 벌어지고, 대시·스톰프는 이를 악물고, 피격·사망은 X눈입니다. 파편·유물을 먹으면 0.4 s 미소가 지나가고(`Actors`가 엔티티의 소비 순간을 감지해 라이브 플레이어에게만 신호), 4 s 넘게 가만히 서 있으면 눈동자가 배회하다 7 s마다 기지개(앞발을 머리 위로, 귀가 쫑긋)를 켭니다. 고양이의 두 귀는 감쇠 스프링으로 몸의 속도 변화를 뒤따르고, 말린 꼬리는 이동 방향과 포즈에 따라 흔들리며, 스카프는 5점 체인이 바이옴 바람(spray 미풍 · rain 역풍 강풍 · spores 느린 흐름 · snow 꾸준한 눈바람)과 달리는 속도에 흐릅니다 — 모두 `PlayerVisual`의 시각 메모리라 리플레이·결정론과 무관합니다. `Skin.accessory`(scarf · antenna · crown · fins · hood · halo · goggles)는 리그와 초상(설정 피커·결과·공유 카드)에서 같이 그려지고, 고양이 스킨은 8종입니다: 클로드·아마조니·엠버·보이드에 **코랄**(`coral`, 지느러미 — 메달 12개), **프로스트**(`frost`, 후드+스카프 — 4층 진입, 정점 구역이 없는 동안은 잠긴 채 남음), **골드**(`gold`, 왕관 — 구역 전부 클리어, 힌트는 `16구역 전부 클리어`처럼 구역 수를 셈), **노바**(`nova`, 후광 — S 등급 3개). 해금·영속·그랜드파더링은 P3-6 경로 그대로입니다. 기본 클로드는 민트색 스카프를 두릅니다. 기존 스킨 ID와 해금 기록은 유지되며, `test/fixtures/clawd-baseline.json`은 고양이로 교체한 네 기본 팔레트의 idle 프레임과 초상을 고정합니다. 적은 공격을 미리 알립니다(`FoeState`만 읽는 시각 전용): 호퍼는 점프 0.3 s 전부터 깊게 웅크리고 눈이 가늘어지며, 포탑은 재장전 마지막 0.4 s에 조준선이 뻗어 나오며 밝아지고, 추격자는 와인드업 동안 떨리고, 플라이어는 날개를 치고, 워커는 제각각 깜빡입니다. **거품**(P5-5, 스폰 `b`)은 히트박스에 정확히 얹힌 반투명 홍채빛 구체로 그려집니다 — 바이옴 accent가 림에 모이는 비누막, 림 위를 도는 홍채 아크, 글린트, 위쪽 크라운 하이라이트('밟아라'), 아랫 림의 희미한 위험색('옆은 죽는다'); 터지면 accent 물방울과 링이 튀고, 사라진 동안은 재생성 0.5 s 전부터 홈 위치로 좁혀 드는 시머 링만 보이며(`bubbleShimmer`), 되살아나면 안쪽으로 닫히는 시머가 붙습니다. 효과음 `bubblePop`(밴드패스 버스트 + 상승 처프, `foeKilled`보다 조용)·`bubbleBack`(글래스 시머)은 이벤트 필드만으로 결정되고, 거품에 죽으면 힌트 '거품은 위에서 밟아라 · 옆에서 닿으면 죽는다'(`deathHint`)가 돕습니다. 오디오는 정점 트랙 `summit`(F# 리디안 · 76 bpm · 글라스 벨 4마디 모티프 · 바람 패드 `wind` 악기 · 소프트 베이스 · 글라스 아르페지오 · 심장 박동)과 클리어 스팅어 `stingSummit`(F#5에서 리디안 B#을 지나 떨어지는 글라스 벨 캐스케이드 — 네 스팅어의 오프닝이 모두 다름)을 얻습니다.
...
levels/         TypeScript DSL + 검증기 + 16개 구역(가로 12 · 세로 4) → src/sim/levels.generated.ts · solutions/(빠른 골든) · solutions/par/(페이스 골든) → src/sim/echoes.generated.ts
...
npm run icons -- --social  # 실행 중인 빌드 → OG 이미지·PWA 스크린샷 (BASE_URL 지정 가능)
```

캐릭터 외형을 바꾼 뒤에는 [캐릭터·아이콘 QA](tools/qa/README.md#character-and-icon-qa)에 따라 SVG와 PNG를 갱신하고, 빌드 화면에서 소셜 이미지를 캡처한 다음 다시 빌드합니다.
...
1. **방을 판다.** `levels/zones/<id>.ts`에서 `room(w, h)`로 시작한다. 좌표는 타일 단위, x는 오른쪽, y는 **아래**로 증가하고 모든 범위는 양끝 포함이다. 스폰 문자는 발판 **바로 위** 빈 칸에 놓는다(런타임이 그 칸의 바닥에 붙인다).
   **크기 봉투와 세로 존(P2-10)**: 구역은 **가로 60~120 × 16~30** 이거나 **세로 36~48 × 60~100**(가로보다 세로가 길면 세로 존으로 읽는다) 둘 중 하나여야 한다(`ZONE_SHAPES`, `zoneShape`). 층마다 네 번째 구역이 세로 존이다 — `t4` 소금 굴뚝(벽에 붙은 4칸 월점프 통로 3개, 선반마다 물웅덩이 안전망), `s4` 천둥 승강기(무너지는 계단 · 세로 승강 발판 `M` 2개 · 토글로 켜는 스위치 블록 계단), `v4` 별빛 우물(상승기류 3단 · 벽을 오르고 우물을 건너 내려와 다시 오르는 크리스탈 체인), `m4` 정점 승강(월점프 통로 1개 · 세로 승강 발판 · 상승기류 3단 · 정상 데크로 건너가는 크리스탈 2개 — 가시가 하나도 없는 탑). 세로 존은 `room(44, h).tower()`로 데일리 타워처럼 양쪽 2칸 벽과 4행 바닥을 두고 시작하고, 벽 옆 통로는 `shaft(2, y0, y1, { leftTop: 0, rightTop, floorDepth })`처럼 기둥 하나만 세운다(`floorDepth`가 없으면 통로 바닥이 방 끝까지 채워져 아래층을 막는다). 세로 존의 체크포인트 간격 규칙은 **등반 순서**(아래 행부터, 같은 행은 직전 마커에서 가까운 쪽부터 — `climbOrder`)로 이웃한 `P`/`C`/`G`의 **맨해튼 거리 ≤ 32칸**이다. 떨어지는 자리는 바위나 물이어야 하고(가시 없는 착지 마당), 체크포인트 두 개를 같은 열에 두지 않는다(`tools/novice.ts`가 체크포인트를 열로 맞춘다). 카메라는 세로 존에서 지면에 서 있을 때 위쪽 리드를 18 더 준다(`CAM_VERTICAL_LEAD`).
   **4층 '오로라 정점'(P5-1, `biome: 'summit'`)**: 새 기믹 없이 기존 문자만으로 짓는다(SIM_VERSION 3 유지). `m1` 얼음 회랑은 일방 발판 `=`을 "아래에서 통과하는 얼음 선반"으로 읽게 하는 복도(천장이 `=`, 끝이 3칸 계단이라 2단 점프로 천장을 뚫고 올라가는 것이 길)와 절벽 면에 붙인 크리스탈 사다리(`D`를 5칸 간격으로 세워 대시 위로 잇기), `m2` 오로라 다리는 무너지는 다리 5칸 + 바위 기둥 2칸의 반복과 가시 침대 위 `m`/`M` 발판·포탑 회랑, `m3` 바람의 첨탑은 v2 패턴의 상승기류(둑 가장자리와 맞닿은 기둥, 가시는 그 너머)를 계단처럼 두 번 쌓고 지붕을 덮은 스위치 회랑(토글은 관문 4~5칸 앞에 — 초보 봇이 벽을 보고 뛰기 전에 대시하도록)과 유물만 지키는 추격자(바닥에서 12행 위, 각성 반경 130 밖)를 둔다. 상승기류 우물 위에는 바위 상인방(`block`)을 얹어 대시 점프로 지붕 위에 올라 회랑을 건너뛰는 길을 막는다. 초보 봇 히트맵의 어느 한 칸도 사망의 25 %를 넘지 않게(호퍼 같은 고정 사망 지점 대신 움직이는 톱날·워커로 분산) 배치한다.
...
3. **스폰 문자.** `ent(ch, x, y)`: `P` 시작 · `G` 골 · `C` 체크포인트 · `o` 파편(`shards([[x,y],…])`, `shardRow`, `arc`는 점프 초대장처럼 포물선으로 놓는다) · `R` 유물 · `S` 용수철 · `D` 대시 크리스탈 · `k` 토글 · 적 `w h f t c` · `b` 거품(밟아 터뜨리면 튀어 오르는 적 — 아래 거품 규칙) · `m`/`M` 수평/수직 발판 · `s` 톱날 · `z` 상승기류(스폰 칸에서 위쪽 첫 바위까지, 최대 12칸).
   ...
   **거품 규칙(P5-5, SIM_VERSION 4)**: `b` 거품은 **위에서만** 다루는 적이다 — 위에서 떨어져 밟거나(`fromAbove`) 공중에서 `{stomp}`를 누른 채 닿으면 터지면서(`bubblePop`) 플레이어를 `PHYS.bubbleBounce`(−352, 스톰프 바운스 −300과 용수철 −430 사이)로 쏘아 올리고 공중 점프와 대시를 리필하지만 **처치는 아니다**(`foeKilled`·`stats.foes`·콤보 없음, 대시 관통은 발사 없이 팝만). 옆이나 아랫면에 닿으면 보통 모드는 **즉사**(사망 원인 `bubble`)고 어시스트 모드는 피격이다. 터진 거품은 2.5초(`PHYS.bubbleRespawn`) 뒤 원래 자리에서 다시 부풀고(`bubbleBack`), 그 자리를 플레이어가 막고 있으면 비켜날 때까지 기다리므로 같은 거품을 2.5초 안에 두 번 쓰는 길은 세우지 않는다. 팝 직후에 점프를 누르면 리필된 공중 점프가 발사를 대신하니(스톰프 바운스와 같은 규칙) 거품 사다리는 **점프를 미리 누른 채** 내려앉는 길로 설계한다. 검증기는 `b` 위 칸이 지형이면(밟고 들어갈 공간이 없다) 거절하고, `P`에서 Chebyshev 거리 6칸 이내면(배우기 전에 만난다) 거절한다. 새 엔티티는 옛 클라이언트가 검증할 수 없으므로 `SIM_VERSION`은 4이며, 스토리 보드는 `s4r*` 키로 새로 열린다.
...
- **빠른 코퍼스 녹화**: `npm run solve -- t1 t2` (인자 없이 실행하면 16구역 전부, 이미 유효한 솔루션은 건너뜀). 옵션 `--budget=300`(구역당 초) `--beam=40` `--random=8` `--seed=1` `--nohurt`(피격도 가지치기) `--force`(재녹화). 솔버는 실제 `Sim`을 복제해 빔 서치를 하고, 찾은 마스크를 **새 Sim으로 `verifyReplay`** 해 통과할 때만 파일을 쓴다. 실패한 구역은 `levels/solutions/PENDING.json`에 이유·최고 진행 지점과 함께 남는다 — 가짜 솔루션을 손으로 만들지 말 것.
...
| `test/levels` | DSL 검증기(봉인된 방·과도한 구덩이 거절, 오류마다 문제 칸 ASCII 덤프, 마크 앵커, 가로/세로 크기 봉투와 세로 존의 등반 순서 간격), 16구역 통과(4층 4개 · 정점 구역별 기믹 단언), 생성 파일 최신 여부, **골든 리플레이**: 빠른 코퍼스는 모든 구역이 사망 0·파 120% 이내로 재생되어 클리어(또는 PENDING 등재), 페이스 코퍼스는 사망 0·파의 95~110 %(또는 par/PENDING 등재 + 클리어)이고 대시가 빠른 클리어보다 적고 파의 20 % 이상 멈춰 있음, `GOAL_ECHOES`는 페이스 코퍼스에서(없으면 빠른 코퍼스로 폴백) |
