...
- **리그 표정·2차 모션·액세서리·스킨 8종·적 예고 포즈·정점 트랙** (P5-2, `src/client/render/clawd.ts`·`actors.ts`, `src/client/unlocks.ts`, `src/client/audio/music.ts`·`sfx.ts`) — 클로드의 얼굴이 포즈를 따라갑니다(`expressionFor`): 달리면 눈썹이 모이고, 떨어지면 눈썹이 오르며 입이 벌어지고, 대시·스톰프는 이를 악물고, 피격·사망은 X눈입니다. 파편·유물을 먹으면 0.4 s 미소가 지나가고(`Actors`가 엔티티의 소비 순간을 감지해 라이브 플레이어에게만 신호), 4 s 넘게 가만히 서 있으면 눈동자가 배회하다 7 s마다 기지개(집게를 머리 위로, 안테나가 쫑긋)를 켭니다. 안테나 2개는 감쇠 스프링으로 몸의 속도 변화를 뒤따르고(대시·착지에 크게 휘어짐), 스카프는 5점 체인이 바이옴 바람(spray 미풍 · rain 역풍 강풍 · spores 느린 흐름 · snow 꾸준한 눈바람)과 달리는 속도에 흐릅니다 — 모두 `PlayerVisual`의 시각 메모리라 리플레이·결정론과 무관합니다. `Skin.accessory`(scarf · antenna · crown · fins · hood · halo · goggles)는 리그와 초상(설정 피커·결과·공유 카드)에서 같이 그려지고, 스킨은 8종이 됩니다: 클로드·아마조니·엠버·보이드에 **코랄**(`coral`, 지느러미 — 메달 12개), **프로스트**(`frost`, 후드+스카프 — 4층 진입, 정점 구역이 없는 동안은 잠긴 채 남음), **골드**(`gold`, 왕관 — 구역 전부 클리어, 힌트는 `16구역 전부 클리어`처럼 구역 수를 셈), **노바**(`nova`, 후광 — S 등급 3개). 해금·영속·그랜드파더링은 P3-6 경로 그대로입니다. 액세서리 없는 기존 4스킨의 idle 프레임과 초상은 이전과 캔버스 호출 단위로 동일하며 `test/fixtures/clawd-baseline.json`이 이를 고정합니다. 적은 공격을 미리 알립니다(`FoeState`만 읽는 시각 전용): 호퍼는 점프 0.3 s 전부터 깊게 웅크리고 눈이 가늘어지며, 포탑은 재장전 마지막 0.4 s에 조준선이 뻗어 나오며 밝아지고, 추격자는 와인드업 동안 떨리고, 플라이어는 날개를 치고, 워커는 제각각 깜빡입니다. 오디오는 정점 트랙 `summit`(F# 리디안 · 76 bpm · 글라스 벨 4마디 모티프 · 바람 패드 `wind` 악기 · 소프트 베이스 · 글라스 아르페지오 · 심장 박동)과 클리어 스팅어 `stingSummit`(F#5에서 리디안 B#을 지나 떨어지는 글라스 벨 캐스케이드 — 네 스팅어의 오프닝이 모두 다름)을 얻습니다.
