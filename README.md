...
- **오디오 연출 패스·캐릭터 주스·세레머니** (P3-9, `src/client/audio/`, `src/client/render/clawd.ts`·`particles.ts`, `src/client/ui/ceremony.ts`) — 골 터치는 재생 중인 바이옴 트랙의 조성으로 쓴 **클리어 스팅어**(E 도리안 마림바 / E 프리지안 스탭 / A 에올리안 벨)를 내고, 체크포인트 차임은 순번마다 한 음씩 오르며, 파편 콤보는 5음계 사다리를 타다 8콤보부터 5도가 겹칩니다. 층 돌파 팡파르·엔딩 화음·별/메달 사운드는 `AudioEngine.stinger`로 sim 이벤트 밖에서 울리고, 엔딩 화면은 타이틀 테마의 변주 트랙 `ending`(F 리디안·64 bpm·글라스 패드)으로 크로스페이드됩니다. 캐릭터는 착지 임팩트에 비례한 먼지, 스킨 색의 대시 잔상 실루엣, 벽 슬라이드 스파크, 골 컨페티, 리스폰 팝을 얻고 골이 6타일 안이면 눈동자가 골을 봅니다(`setLookTarget`, 라이브 플레이어만). 결과 화면은 `Timeline`이 등급 → 별 → 메달 행(`LevelRecord.medals`) → 보드 순으로 `data-stage`를 올려 CSS가 단계별로 드러내며(아무 키로 완료, `prefers-reduced-motion`과 `?shot=` 하네스는 즉시), 한 층의 마지막 구역을 처음 돌파하면 결과 전에 3초짜리 **층 돌파 비스타 카드**(`#scr-tier`, 바이옴 그라디언트·로마 숫자·한 줄 서사·다음 층, 아무 키 스킵)가 한 번 나오고 `Progress.tiersBroken`에 남습니다. 9개 구역을 모두 처음 돌파하면 결과 대신 **엔딩**(`#scr-ending`: 절차적 밤하늘 캔버스, 정상의 클로드, 3줄 서사, 총 별·유물·사망·최고 기록 합, 이 클리어의 제출 줄, '다시 오르기' → 탑)이 한 번 나오고 `Progress.endingSeen` 이후 타이틀에 한 줄 서사가 붙습니다. 두 플래그는 이전 코드 스냅샷에 실려 병합되고, 진행 기록 삭제로 지워집니다. 텔레메트리는 기존 화면 이름만 씁니다(엔딩은 `result`).
...
10. **플레이 테스트.** `npm run dev` → 구역 선택에서 열어 본다(해금 규칙은 앞 구역 클리어). `qa:smoke`가 9구역을 `?shot=`으로 캡처하니 새 구역은 `tools/qa/smoke.ts`의 `ZONES` 목록에도 넣는다.
