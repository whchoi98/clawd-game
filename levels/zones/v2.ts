 ...
 *
 * P2-8 (rev 1): five checkpoints — cliff top, maze floor, mid-maze ledge, the
 * exit wall top and the first climax ledge (the flyers there and in the maze
 * were 99 % of novice deaths) — and ten shards off the line: a dash-up perch
 * at the start, inside both shortcut columns, beside the relic, and a dash-up
 * perch above the climax.
 */
...
// --- start yard: a perch six up
m.ground(0, 16, F);
m.ent('P', 3, F - 1);
m.plat(10, 11, F - 7);
m.shards([[10, F - 8], [11, F - 8]]);
...
for (const [x0, x1, y] of ledges) m.owp(x0, x1, y);
m.owp(57, 60, F - 18);                        // rest ledge between the top ledge and the exit wall top
m.ent('C', 38, F - 1);                        // maze floor
m.ent('C', 50, F - 12);                       // mid-maze, on the fourth ledge
m.ent('f', 45, F - 10);
m.ent('f', 50, F - 15);
m.ent('z', 58, F - 1);                        // shortcut column up the right side of the maze
m.ent('z', 36, F - 1);                        // and one up the left side
m.shards([[36, F - 8], [36, F - 14], [58, F - 6], [58, F - 12]]);   // ride the shortcuts to collect
m.ent('C', 64, F - 20);                       // on the exit wall top

// --- relic: a ledge left of the maze, five above the cliff — only reachable from the maze
m.plat(31, 33, F - 15);
m.ent('R', 32, F - 16);
m.shards([[31, F - 16], [33, F - 16]]);

// --- climax: descend over a spike floor on three ledges past two flyers; a dash-up perch above the first ledge
m.ground(67, 89, F);
m.spikes(67, 89, F - 1);
m.plat(70, 74, F - 15);
m.ent('C', 72, F - 16);
m.plat(77, 78, F - 20);
m.shards([[77, F - 21], [78, F - 21]]);
m.ent('f', 76, F - 18);
m.plat(78, 82, F - 10);
m.ent('f', 84, F - 13);
m.plat(86, 90, F - 5);
