...
[화면·안정성 개선](2026-09-13-premium-report.md)에 이어, 이미 게임에 있던 메달과 모습을 다시 오를 이유로 연결했다. 이 단계에서 검증한 빌드는 **`bd8f1cb4`**이며, 당시 변경분은 **Unreleased** 상태였다.

위 버전과 수치는 배포 전 검증 시점의 기록이다. 이후 **v0.6.0 · b7a69cd7**로 배포했으며, 실제 서비스 검증은 [릴리스 보고서](2026-09-13-release-0.6.0.md)에 있다.
