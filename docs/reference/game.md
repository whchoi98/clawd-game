...
Versions have separate responsibilities: package **0.7.0**, **SIM_VERSION 4**, **GEN_VERSION 2**, settings/progress schema **v1**.
...
| Character | [clawd.ts](../../src/client/render/clawd.ts) draws cat, rabbit and robot shapes, their ten appearance entries, portraits, and dash silhouettes. Three starter characters are available immediately; seven earned cat costumes retain their rules. [PlayerVisual](../../src/client/render/actors.ts) supplies ear springs and the scarf chain. The portrait and ending UI share the `PORTRAIT_FEET` anchor from [contracts.ts](../../src/client/contracts.ts). Existing skin ids, unlocks, hitboxes, and replay versions remain compatible. Follow [character and icon QA](../../tools/qa/README.md#character-and-icon-qa) for visual checks, SVG updates, PNG generation, and social preview capture. |
...
패키지 **0.7.0**, **SIM_VERSION 4**, **GEN_VERSION 2**, 설정·진행도 스키마 **v1**은 각각 다른 변경 범위를 담당합니다.
...
| 캐릭터 | [clawd.ts](../../src/client/render/clawd.ts)가 고양이·토끼·로봇 형태와 총 열 가지 모습, 초상, 대시 실루엣을 그립니다. 기본 캐릭터 3종은 즉시 선택할 수 있고 고양이 모습 7종의 획득 조건은 유지합니다. [PlayerVisual](../../src/client/render/actors.ts)은 귀 스프링과 스카프 체인을 제공합니다. 초상과 엔딩 UI는 [contracts.ts](../../src/client/contracts.ts)의 `PORTRAIT_FEET` 기준점을 공유합니다. 기존 스킨 ID·해금·히트박스·리플레이 버전과 호환됩니다. 외형 확인, SVG 수정, PNG 생성과 공유 미리보기 캡처는 [캐릭터·아이콘 QA](../../tools/qa/README.md#character-and-icon-qa)를 따릅니다. |
